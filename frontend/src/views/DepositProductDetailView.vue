<template>
  <div v-if="financialStore.depositProduct" class="detail-container">
    <h2 class="title">정기예금 상세</h2>

    <div class="detail-card">
      <div class="detail-row detail-margin-bottom">
        <div class="detail-col-3"><strong>공시 제출월</strong></div>
        <div class="detail-col-9">{{ financialStore.depositProduct.dcls_month }}</div>
      </div>
      <div class="detail-row detail-margin-bottom">
        <div class="detail-col-3"><strong>금융회사명</strong></div>
        <div class="detail-col-9">{{ financialStore.depositProduct.kor_co_nm }}</div>
      </div>
      <div class="detail-row detail-margin-bottom">
        <div class="detail-col-3"><strong>상품명</strong></div>
        <div class="detail-col-9">{{ financialStore.depositProduct.fin_prdt_nm }}</div>
      </div>
      <div class="detail-row detail-margin-bottom">
        <div class="detail-col-3"><strong>가입제한</strong></div>
        <div class="detail-col-9">{{ financialStore.depositProduct.join_deny }}</div>
      </div>
      <div class="detail-row detail-margin-bottom">
        <div class="detail-col-3"><strong>가입방법</strong></div>
        <div class="detail-col-9">{{ financialStore.depositProduct.join_way }}</div>
      </div>
      <div class="detail-row detail-margin-bottom">
        <div class="detail-col-3"><strong>우대조건</strong></div>
        <div class="detail-col-9" v-html="financialStore.depositProduct.spcl_cnd"></div>
      </div>
      <div class="detail-row detail-margin-bottom">
        <div class="detail-col-3"><strong>저축 기간 및 금리</strong></div>
        <div class="detail-col-9">
          <ul class="no-bullet-list">
            <li v-for="option in financialStore.depositProduct.options" :key="option.id">
              {{ option.save_trm }}개월: 기본 {{ option.intr_rate }}%, 우대 {{ option.intr_rate2 }}%
            </li>
          </ul>
        </div>
      </div>
      <div class="detail-row detail-margin-bottom">
        ...
        <div class="detail-col-9" v-html="financialStore.depositProduct.mtrt_int"></div>
      </div>
      <div class="detail-row detail-margin-bottom">
        <div class="detail-col-3"><strong>최대 한도</strong></div>
        <div class="detail-col-9">{{ financialStore.depositProduct.max_limit }}</div>
      </div>
      ...
      <div class="modal-body-custom">
        <form @submit.prevent="submitJoin">
          <div class="form-group detail-margin-bottom">
            <label for="principal" class="form-label-custom">원금:</label>
...
const currentProductOptions = computed(() => {
  return financialStore.depositProduct?.options || []; // 예금 페이지이므로 depositProduct만 확인
});
...
const updateAppliedRate = () => {
  ...
  if (selectedOption) {
    joinPayload.value.applied_rate = selectedOption.intr_rate2; // 우대 금리를 적용 금리로 사용
  } else {
...
watch(() => route.params.id, (newId) => {
  if (newId) {
    financialStore.fetchDepositProductDetail(newId);
  }
...
const submitJoin = async () => {
  try {
    ...
    await financialStore.subscribeDepositProduct(route.params.id, joinPayload.value);
