<template>
  <div class="container mt-5">
    ...
    <table class="table table-hover">
      ...
      <tbody>
        <tr v-for="product in financialStore.savingProducts" :key="`${product.id}-${product.save_trm}`" @click="goToDetail(product.id)">
...
onMounted(() => {
  ...
  // 초기 적금 상품 목록을 불러와 dclsMonths를 채웁니다.
  financialStore.fetchSavingProducts(); 
});
...
const applyFilters = () => {
  ...
  financialStore.fetchSavingProducts(params);
};

const goToDetail = (id) => {
  router.push({ name: 'SavingProductDetailView', params: { id } }); // 상세 페이지로 이동
};
</script>
